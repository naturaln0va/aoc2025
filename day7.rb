# This script was written for the advent of code 2025 day 7.
...
class Solver
  ...
  def solve_first(input)
    ...
    t_positions = Array.new
    grid.each_with_index do |row, y|
      temp_pos = t_positions.clone
      t_positions.each do |pos|
        val = row[pos]
        if val == "^"
          temp_pos.delete(pos)
          temp_pos << pos - 1
      ...
      t_positions = temp_pos.uniq.sort

      if t_positions.empty?
        start = row.find_index("S")
        t_positions << start
      end
    end
