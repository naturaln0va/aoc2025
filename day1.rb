# This script was written for the advent of code 2025 day 1.
...
class Solver
  ...
  def solve_first(input)
    ...
    cmds.each do |cmd|
      dir = cmd.slice!(0)
      to_move = cmd.to_i % 100
      if dir == "L"
        # puts "turning left #{cmd}"
        dial -= to_move
      else
        # puts "turning right #{cmd}"
        dial += to_move
      end

      if dial < 0
        adjusted = 100 + dial
        # puts "adjusting #{dial} to #{adjusted}"
        dial = adjusted
      elsif dial > 99
        adjusted = dial - 100
        # puts "adjusting #{dial} to #{adjusted}"
